...
class NormFormDemo extends AbstractNormForm
{
    ...
    public function __construct(View $defaultView)
    {
        ...
        $this->guard->validateStorage();
        $this->csrf_name  = $this->guard->getTokenNameKey();
        $this->csrf_value = $this->guard->getTokenValueKey();
        $keyPair = $this->guard->generateToken();
        $this->currentView->setParameter(new GenericParameter("csrf_name", $this->csrf_name));
        $this->currentView->setParameter(new GenericParameter("csrf_name_token", $keyPair[$this->csrf_name]));
        $this->currentView->setParameter(new GenericParameter("csrf_value", $this->csrf_value));
        $this->currentView->setParameter(new GenericParameter("csrf_value_token", $keyPair[$this->csrf_value]));
    }
    ...
    protected function isValid(): bool
    {
        ...
        if (!$this->guard->validateToken($_POST[$this->csrf_name], $_POST[$this->csrf_value])) {
            $this->errorMessages[$this->csrf_name] = "CSRF Token wrong.";
        }
